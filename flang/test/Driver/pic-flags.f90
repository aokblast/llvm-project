...
! RUN: %if x86_64-registered-target && clang_default_pie_on_freebsd %{ %flang -v -S -emit-llvm -o - %s --target=x86_64-unknown-freebsd 2>&1 | FileCheck %s --check-prefixes=CHECK,CHECK-PIE-LEVEL2,CHECK-PIE-LEVEL2-IR %}
